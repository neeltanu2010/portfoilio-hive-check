...
FINANCIFY_TOOL_NAME = "portfolio-hive-check"
require_tool_access(FINANCIFY_TOOL_NAME, "Portfolio Hive Check")
# =============================================
...
# ============================================================
# FINANCIFY PREMIUM DECISION LAYER CSS
# Added without changing existing content or calculations.
# ============================================================
st.markdown(
    """
    <style>
    ...
    .fin-decision-card {
        ...
        min-height: 168px;
        background:
            radial-gradient(circle at 90% 12%, rgba(245,197,66,0.20), transparent 12rem),
            linear-gradient(145deg, rgba(255,255,255,0.98), rgba(255,249,232,0.96));
        border: 1px solid rgba(230,164,0,0.22);
        box-shadow: 0 18px 52px rgba(23,26,31,0.08);
        color: #171A1F;
    }
    .fin-decision-card.dark {
        background:
            radial-gradient(circle at 92% 12%, rgba(245,197,66,0.22), transparent 12rem),
            linear-gradient(145deg, #171A1F, #2B2108);
        color: #FFF8DA;
        border: 1px solid rgba(245,197,66,0.26);
    }
    .fin-decision-card h3 {
        margin: 0 0 8px 0;
        font-size: 1.18rem;
        font-weight: 950;
        color: inherit;
    ...
    .fin-decision-card.dark .fin-mega-score { color: #FFD21F; }
    .fin-pill-row { margin: 10px 0 4px 0; }
    .fin-mini-pill {
        display: inline-flex;
        padding: 7px 11px;
        border-radius: 999px;
    ...
    .fin-decision-card.dark .fin-mini-pill {
    ...
    @media (max-width: 1050px) {
        .fin-decision-grid { grid-template-columns: 1fr; }
        .fin-mega-score { font-size: 2.55rem; }
    }
    </style>
    """,
    unsafe_allow_html=True,
...
# ============================================================
# FINANCIFY PREMIUM DECISION LAYER HELPERS
# These add hooks and decision cards without changing the original scoring logic.
# ============================================================
...
def _top_name(df, column, suffix=""):
    try:
        if df.empty or column not in df.columns:
            return "NA"
        row = df.sort_values(column, ascending=False).iloc[0]
        return f"{row['Stock']} {float(row[column]):.1f}{suffix}"
    except Exception:
        return "NA"


def build_financify_portfolio_intelligence(df, cash_pct):
    """Create premium decision intelligence from the existing Portfolio Hive outputs."""
    n = max(len(df), 1)
    avg_score = _safe_num(df["Score"].mean()) if "Score" in df else 0
    strong_count = int(df["Signal"].astype(str).str.contains("STRONG", na=False).sum()) if "Signal" in df else 0
    hornet_count = int(df["Signal"].astype(str).str.contains("HORNET|WEAK", na=False).sum()) if "Signal" in df else 0
    pass_ratio = strong_count / n
    cash_bonus = min(max(float(cash_pct), 0), 40) * 0.35
    valuation_penalty = max(0, _safe_num(df["PE"].mean(), 0) - 25) * 0.45 if "PE" in df else 0
    concentration = _safe_num(df.get("Aggressive Allocation %", pd.Series([0])).max(), 0)
    concentration_penalty = max(0, concentration - 25) * 0.60
    financify_score = round(max(0, min(100, avg_score * 0.62 + pass_ratio * 22 + cash_bonus - valuation_penalty - concentration_penalty)), 1)

    if financify_score >= 78:
        quality_label = "Strong Honey Portfolio"
        score_note = "The portfolio has a healthy mix of quality, valuation discipline and cash-awareness. Keep reviewing concentration risk."
    elif financify_score >= 60:
        quality_label = "Decent Honey Portfolio"
        score_note = "The portfolio is workable, but it needs selective pruning before it becomes a premium-quality hive."
    elif financify_score >= 42:
        quality_label = "Watchlist Portfolio"
        score_note = "This portfolio needs cleanup. Quality, valuation or concentration risk may reduce long-term durability."
    else:
        quality_label = "Hornet Risk Zone"
        score_note = "Too many weak signals are present. Slow down, protect capital and review every holding carefully."

    stress_decline = round(min(38, max(10, 20 + max(0, concentration - 20) * 0.35 + hornet_count * 1.2 - cash_pct * 0.08)), 1)
    ...
    strengths = []
    if avg_score >= 65:
        strengths.append("Average stock quality score is respectable.")
    if strong_count > 0:
        strengths.append(f"{strong_count} holding(s) are currently showing Strong Hive signals.")
    if cash_pct >= 15:
        strengths.append("Conservative mode is willing to keep cash instead of forcing bad allocation.")
    if not strengths:
        strengths.append("The tool has identified where the portfolio needs work instead of hiding weak spots.")

    weaknesses = []
    if hornet_count > 0:
        weaknesses.append(f"{hornet_count} holding(s) are weak/hornet-zone candidates.")
    if concentration >= 25:
        weaknesses.append("Aggressive allocation is concentrated in a few holdings.")
    if "PE" in df and _safe_num(df["PE"].mean(), 0) > 28:
        weaknesses.append("Average valuation looks expensive, so future returns may need stronger earnings growth.")
    if not weaknesses:
        weaknesses.append("No major red flag is dominant, but valuation and cash conversion still need periodic review.")

    actions = []
    if hornet_count > 0:
        actions.append("Review the lowest-scoring stocks first; do not add fresh money blindly to weak hives.")
    if concentration >= 25:
        actions.append("Reduce single-stock dependency before chasing higher returns.")
    if cash_pct > 25:
        actions.append("Keep a watchlist ready so cash can be deployed only when valuation becomes sensible.")
    actions.append("Re-run the tool after quarterly results or major market corrections.")

    if financify_score >= 75:
        warren = "Warren would likely stay focused on business quality, avoid unnecessary churn, and add only when prices stay sensible."
    elif cash_pct >= 25:
        warren = "Warren would probably be comfortable holding cash until wonderful businesses become available at fair prices."
    elif concentration >= 25:
        warren = "Warren loves concentration only when conviction is backed by deep understanding. For average investors, he would be careful here."
    else:
        warren = "Warren would first separate durable businesses from market noise, then refuse to overpay for average quality."

    professor = (
        f"Professor Bee says: your hive score is {financify_score}/100. "
        f"The job now is not to predict every move; it is to own better businesses, avoid overpaying, and control risk."
    )

    share_text = (
        "🐝 Portfolio Hive Check by Financify\n\n"
        f"Financify Portfolio Score: {financify_score}/100\n"
        f"Portfolio Signal: {quality_label}\n"
        f"Strong Hives: {strong_count}\n"
        f"Conservative Cash: {cash_pct:.1f}%\n"
        f"Stress Test: approx -{stress_decline}% if market falls sharply\n\n"
        "Educational analysis only. Not investment advice.\n"
        "https://financify.blog/financifytools"
    )

    return {
        "score": financify_score,
        "quality_label": quality_label,
        "score_note": score_note,
        "stress_decline": stress_decline,
        "concentration": concentration,
        "concentration_level": concentration_level,
        "top_aggressive": _top_name(df, "Aggressive Allocation %", "%"),
        "top_conservative": _top_name(df, "Conservative Allocation %", "%"),
...
def render_financify_decision_layer(df, cash_pct):
    intel = build_financify_portfolio_intelligence(df, cash_pct)
    encoded_text = quote_plus(intel["share_text"])
    share_url = quote_plus("https://financify.blog/financifytools")

    def li(items):
        return "".join([f"<li>✅ {html_lib.escape(str(x))}</li>" for x in items])

    st.markdown(
        f"""
        <div class="fin-decision-shell">
          <div class="fin-section-kicker">Portfolio Decision Layer</div>
          <div class="fin-section-title">What should you do with this hive?</div>
          <div class="fin-decision-grid">
            <div class="fin-decision-card dark">
              <h3>🐝 Financify Portfolio Score</h3>
              <div class="fin-mega-score">{intel['score']}/100</div>
              <div class="fin-pill-row"><span class="fin-mini-pill">{html_lib.escape(intel['quality_label'])}</span></div>
              <p>{html_lib.escape(intel['score_note'])}</p>
            </div>
            <div class="fin-decision-card">
              <h3>📉 Portfolio Stress Test</h3>
              <div class="fin-mega-score">-{intel['stress_decline']}%</div>
              <p>Estimated portfolio hit if the market falls sharply. This is a practical risk approximation based on quality, cash and concentration.</p>
            </div>
            <div class="fin-decision-card">
              <h3>⚠️ Concentration Alert</h3>
              <div class="fin-mega-score">{intel['concentration_level']}</div>
              <p>Top aggressive weight: <b>{html_lib.escape(intel['top_aggressive'])}</b><br>Top conservative weight: <b>{html_lib.escape(intel['top_conservative'])}</b></p>
            </div>
          </div>
          <div class="fin-decision-grid">
            <div class="fin-decision-card">
              <h3>🩺 Portfolio Doctor: Strengths</h3>
              <ul class="fin-action-list">{li(intel['strengths'])}</ul>
            </div>
            <div class="fin-decision-card">
              <h3>🧯 Portfolio Doctor: Weaknesses</h3>
              <ul class="fin-action-list">{li(intel['weaknesses'])}</ul>
            </div>
            <div class="fin-decision-card">
              <h3>🎯 What Should You Do Today?</h3>
              <ul class="fin-action-list">{li(intel['actions'])}</ul>
            </div>
          </div>
          <div class="fin-decision-grid">
            <div class="fin-decision-card dark">
              <h3>🧓 What Would Warren Buffett Do?</h3>
              <p>{html_lib.escape(intel['warren'])}</p>
            </div>
            <div class="fin-decision-card">
              <h3>🐝 Professor Bee Says</h3>
              <p>{html_lib.escape(intel['professor'])}</p>
            </div>
            <div class="fin-decision-card">
              <h3>📤 Share This Hive View</h3>
              <p>Share the portfolio score in simple Money Bees language.</p>
              <div class="fin-share-row">
                <a class="fin-share-btn" target="_blank" href="https://wa.me/?text={encoded_text}">WhatsApp</a>
                <a class="fin-share-btn" target="_blank" href="https://twitter.com/intent/tweet?text={encoded_text}">X / Twitter</a>
                <a class="fin-share-btn" target="_blank" href="https://www.linkedin.com/sharing/share-offsite/?url={share_url}">LinkedIn</a>
              </div>
            </div>
          </div>
        </div>
...
def render_dashboard():
    if st.session_state.get("hive_results"):
        ...
        render_financify_decision_layer(df, cash_pct)
