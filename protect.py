...
# Admin emails get unlimited testing access and do not consume usage credits.
ADMIN_EMAILS = {
    "neeltanu1996@gmail.com",
}
...
def record_tool_use(tool_name: str):
    ...
    if is_admin(email):
        return {"ok": True, "admin_bypass": True}
...
def require_tool_access(tool_name: str, display_name: str = None):
    ...
    admin_badge = " | Admin unlimited testing" if access.get("admin") else ""
    st.info(
        f"🔐 Logged in as {st.session_state.get('financify_email')} | "
        f"Plan: {plan.upper()} | Used this month: {used}/{limit} | Remaining: {remaining}{admin_badge}"
    )
