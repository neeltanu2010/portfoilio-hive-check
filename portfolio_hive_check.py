...
def run_portfolio_backtest(df, allocation_column, start_date, end_date, initial_value=100):
    """Backtest current allocation weights against Nifty 50 and Sensex."""
    ...
    if "^NSEI" in prices.columns:
        nifty = prices["^NSEI"].reindex(curves.index).ffill().dropna()
        if len(nifty) > 2:
            curves["Nifty 50"] = (nifty / nifty.iloc[0]) * initial_value

    if "^BSESN" in prices.columns:
        sensex = prices["^BSESN"].reindex(curves.index).ffill().dropna()
        if len(sensex) > 2:
            curves["Sensex"] = (sensex / sensex.iloc[0]) * initial_value
    ...
    note = ""
    if missing:
        note = "Missing price data ignored for: " + ", ".join([x.replace(".NS", "") for x in missing])
    return curves, metrics_df, note


def plot_backtest_curves(curves):
    fig = go.Figure()
    for col in curves.columns:
        fig.add_trace(go.Scatter(x=curves.index, y=curves[col], mode="lines", name=col, line=dict(width=3)))
    fig.update_layout(
        title="Portfolio vs Nifty 50 vs Sensex",
        xaxis_title="Date",
...
def plot_metric_bars(metrics_df, metric_col, title, suffix=""):
    fig = go.Figure()
    fig.add_trace(go.Bar(x=metrics_df["Asset"], y=metrics_df[metric_col], text=[f"{v:.2f}{suffix}" if pd.notna(v) else "NA" for v in metrics_df[metric_col]], textposition="outside"))
    fig.update_layout(
        title=title,
        xaxis_title="",
...
if st.session_state.get("hive_results"):
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Summary", "🛡️ Conservative", "🚀 Aggressive", "📉 Backtester", "🔍 Deep Check"])
    ...
    with tab4:
        ...
        st.markdown(
            ...
            <div class="info-card">
                ...
                It uses the allocation generated by <b>Portfolio Hive Check</b> and compares the portfolio with <b>Nifty 50</b> and <b>Sensex</b> over the selected crash period. The chart starts at ₹100 so the comparison stays clean.
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if st.button("Run Backtest", type="primary", key="run_backtester"):
            if pd.to_datetime(start_date) >= pd.to_datetime(end_date):
                ...
                with st.spinner("Backtesting the hive against market storms..."):
                    curves, metrics_df, bt_note = run_portfolio_backtest(df, allocation_column, start_date, end_date, initial_value=100)
                ...
                if curves.empty or metrics_df.empty:
                    ...
                    st.session_state["backtest_title"] = f"{allocation_mode} | {period_choice} | {start_date} to {end_date}"
